...
namespace YogitaFashionAPI.Controllers
{
    ...
    public class ProductsController : ControllerBase
    {
        ...
        public async Task<IActionResult> CreateProduct([FromBody] Product input)
        {
            ...
            await _auditLogService.WriteAsync(User, HttpContext, "ProductCreate", "Product", product.Id.ToString(), "Success", new Dictionary<string, object?>
            {
                ["name"] = product.Name,
                ["stock"] = product.Stock,
                ["price"] = product.Price
        ...
        public async Task<IActionResult> UpdateProduct(int id, [FromBody] Product input)
        {
            ...
            await _auditLogService.WriteAsync(User, HttpContext, "ProductUpdate", "Product", existing.Id.ToString(), "Success", new Dictionary<string, object?>
            {
                ["name"] = existing.Name,
        ...
        public async Task<IActionResult> DeleteProduct(int id)
        {
            ...
            {
                return Conflict(new { message = "Product is referenced by existing orders or returns and cannot be deleted." });
            }

            await _auditLogService.WriteAsync(User, HttpContext, "ProductDelete", "Product", existing.Id.ToString(), "Success", new Dictionary<string, object?>
            {
                ["name"] = existing.Name
            });
            return NoContent();
        ...
        public async Task<IActionResult> UploadImage([FromForm] IFormFile? file)
        {
            ...
            {
                await _auditLogService.WriteAsync(User, HttpContext, "ProductImageUpload", "ProductImage", cloudUpload.PublicId, "Success", new Dictionary<string, object?>
                {
                    ["provider"] = "cloudinary",
            ...
            await _auditLogService.WriteAsync(User, HttpContext, "ProductImageUpload", "ProductImage", fileName, "Success", new Dictionary<string, object?>
            {
                ["provider"] = "local",
                ["sizeBytes"] = file.Length
            });
        ...
        public async Task<IActionResult> SubscribeStockAlert(int id, [FromBody] CreateStockAlertRequest request)
        {
            ...
            var duplicate = await _db.StockAlertSubscriptions.FirstOrDefaultAsync(subscription =>
                ...
                (subscription.Email ?? "").ToLower() == email &&
                (subscription.WhatsAppNumber ?? "") == whatsAppNumber &&
        ...
        public async Task<IActionResult> NotifyStockAlerts(int id)
        {
            ...
            await _auditLogService.WriteAsync(User, HttpContext, "StockAlertNotify", "StockAlert", id.ToString(), "Success", new Dictionary<string, object?>
            {
        ...
        private static ProductDetails NormalizeDetails(ProductDetails? input)
        {
            var details = input ?? new ProductDetails();
            return new ProductDetails
            {
                Material = (details.Material ?? "").Trim(),
                Fit = (details.Fit ?? "").Trim(),
                Style = (details.Style ?? "").Trim(),
                Sleeve = (details.Sleeve ?? "").Trim(),
                WashCare = (details.WashCare ?? "").Trim()
            };
        }
        ...
        private async Task<bool> TrySendEmailNotification(Product product, StockAlertSubscription subscriber)
        {
            ...
            var body =
                ...
                $"Product link: {_notificationService.GetStoreFrontBaseUrl()}/product/{product.Id}\n\n" +
                "You can place your order now.\n\nThank you.";

            return await _notificationService.SendEmailAsync(subscriber.Email, subject, body);
        }
        ...
        private async Task<bool> TrySendWhatsAppNotification(Product product, StockAlertSubscription subscriber)
        {
            ...
            if (simulate)
            {
                _logger.LogInformation(
                    ...
                    product.Name,
                    _notificationService.GetStoreFrontBaseUrl(),
                    product.Id
                );
        ...
        private async Task<int> NotifySaleForProduct(Product product)
        {
            ...
            foreach (var recipient in recipients)
            {
                ...
                var body =
                    ...
                    $"Buy now: {_notificationService.GetStoreFrontBaseUrl()}/product/{product.Id}\n\n" +
                    "Thank you.";

                if (await _notificationService.SendEmailAsync(recipient, subject, body))
                {
                    sentCount++;
                }
            }
